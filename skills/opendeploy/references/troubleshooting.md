...
|---|---|---|
| `npm view @opendeploydev/cli version` fails during explicit setup/update/latest audit with `ENOTFOUND registry.npmjs.org`, `EAI_AGAIN`, timeout, or proxy/connect errors | npm registry path is blocked from the agent process, or the host has not granted network access | use `opendeploy-setup` -> "Agent Network / PATH / Proxy Repair"; retry once with approved network/escalated execution when available; do not keep reinstalling or switch to `npx`. Normal deploy preflight should use dashboard `/v1/status` policy and does not need this npm probe |
...
| Live URL returns plain HTTP 403, service is active, runtime is `vite preview` | Vite preview rejected the public `*.opendeploy.site` Host header | patch `vite.config.*` `preview.allowedHosts` when this fix was listed in the approved deploy plan; otherwise ask once, then re-upload and redeploy once |
